#include<bits/stdc++.h>
using namespace std; 
#define hashFunction1(k) (2*k)+3
#define hashFunction2(k) (3*k)+1
...
int main(int argc, char const *argv[])
{
	...
	{
		int key=keys[i];
		int location=(hashFunction1(key))%hashSize;
		...
		{
			int probe=0,k=0,v=(hashFunction2(key))%hashSize;
			if(v==0) 
			{
			...
			{
				for(int j=((location + k*v)%hashSize); ; j=((location + k*v)%hashSize))
				{
					probe++;
					k++;
